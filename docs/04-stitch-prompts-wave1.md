...
> # ⛔ وثيقة تاريخية — متجاوَزة، لا تُستعمل مرجعاً
>
> **الحالة:** مؤرشفة 2026-08-20 · **لا تنسخ منها أي قيمة لون أو تخطيط.**
>
> بُنيت هذه البرومتات قبل اعتماد الهوية البصرية، وقد أدّت غرضها: الموجة الأولى (الشاشات 1–3) مبنية فعلياً منذ ذلك الحين. محتواها الآن **يناقض `DESIGN.md` مناقضة مباشرة** في نقطتين جوهريتين:
>
> | ما تقوله هذه الوثيقة | الواقع المعتمد في [DESIGN.md](../DESIGN.md) v2.3 |
> |---|---|
> | الفعل الأساسي `#0a0a0a` أسود | `accent` **`#525fe1`** بنفسجي |
> | لون مميّز `#00d4a4` نعناعي | `amber` **`#f0a500`** كهرماني |
> | لون كسر إيقاع `#f55a3c` برتقالي | **غير موجود** — النظام لونان كروماتيّان فقط |
> | الأرضية `#ffffff` أبيض ناصع | `ground` **`#f5f8f7`** — الأبيض الناصع مرفوض صراحةً |
> | النص `#0a0a0a` / `#1c1c1e` | `ink` **`#0b104a`** / `charcoal` **`#1a2d62`** |
> | صفحة المادة بثلاثة أعمدة | **أُبطِل** — استُبدل بنمط Hero + عمودين ([docs/03](03-screens-detail.md) §3) |
>
> **مصدر الحقيقة الوحيد للتصميم هو [`DESIGN.md`](../DESIGN.md) وصفحة `/styleguide` الحيّة.**
> تُركت هذه الوثيقة كما هي لقيمتها التأريخية (كيف فكّرنا في المرحلة الأولى) لا لقيمتها المرجعية.

---

<details>
<summary><strong>المحتوى الأصلي (مؤرشف — اضغط للعرض)</strong></summary>
...
## ما حدث فعلاً بعد هذه الوثيقة

الشاشات الثلاث بُنيت في Blade + Tailwind 4 مباشرة، لا من مخرَج Stitch. اللوحة اللونية استُبدلت بالكامل بلوحة **edunova** المعتمدة، وصفحة المادة أُعيد تصميمها من نمط الأعمدة الثلاثة إلى **Hero + عمودين** بعد أن بدا النمط الثلاثي مجزّأً في المراجعة الفعلية.

الدور الذي كانت تؤدّيه هذه الوثيقة (سطح للاتفاق البصري قبل الكود) تؤدّيه اليوم صفحة **`/styleguide`** الحيّة — وهي أدق منها لأنها تعرض المكوّنات الحقيقية بالـtokens الحقيقية في الاتجاهين، لا محاكاة خارجية.
